...
const auto = {
  ...
  agent(id: string, spec: Agent) {
    ...
    append("system", { kind: "AgentOffered", agentId: id, agentKind: spec.kind, skills: spec.provides });
  },
...
auto.skill("ship-feature", {
  description: "Ship a dashboard feature. Requires code, CI pass, cast recording, and 2 manual reviews.",
  requires_proof: {
...
const EMOJI: Record<string, string> = {
  SkillRegistered: "📘", AgentOffered: "🤝", RoleAttested: "📜",
  DispatchProposed: "📮", DispatchClaimed: "🙋", ArtifactUploaded: "🔨",
  ArtifactValidated: "✔️ ", ProofAccepted: "🎖️ ", DispatchConfirmed: "🟢",
  DispatchBlocked: "🔒", DispatchApproved: "✅",
};
subs.push((f) => {
  const e = EMOJI[f.payload.kind] ?? "•";
  const tag = f.payload.kind;
  const detail = f.payload.skillId ? `skill=${f.payload.skillId}` :
                 f.payload.artifactType ? `type=${f.payload.artifactType} hash=${f.payload.hash ?? f.payload.artifactHash}${f.payload.verdict ? ` verdict=${f.payload.verdict}` : ""}` :
                 f.payload.dispatchId ? `id=${f.payload.dispatchId}` :
                 f.payload.role ? `${f.payload.subject}:${f.payload.role}` :
                 f.payload.agentId ? `agent=${f.payload.agentId}` : "";
  const reject = f.rejectedReason ? `  🔒 REJECTED — ${f.rejectedReason}` : "";
  console.log(`  [L${String(f.lamport).padStart(3)}] ${f.signer.padEnd(18)} ${e} ${tag.padEnd(19)} ${detail}${reject}`);
});
...
append("jacob-laptop", { kind: "DispatchProposed", dispatchId, skillId: "ship-feature", payload: { spec: "LINEAR-123" } });
append("claude-builder", { kind: "DispatchClaimed", dispatchId, byAgent: "claude-builder" });

append("claude-builder", { kind: "ArtifactUploaded", dispatchId, artifactType: "code", hash: "abc123", uri: "s3://artifacts/code-abc123.tar.gz" });
append("github-ci",      { kind: "ArtifactValidated", dispatchId, artifactHash: "abc123", artifactType: "code", verdict: "accepted" });

append("claude-builder", { kind: "ArtifactUploaded", dispatchId, artifactType: "ci-result", hash: "def456", uri: "github.com/.../run/9876" });
append("github-ci",      { kind: "ArtifactValidated", dispatchId, artifactHash: "def456", artifactType: "ci-result", verdict: "accepted" });

append("claude-builder", { kind: "ArtifactUploaded", dispatchId, artifactType: "cast", hash: "ghi789", uri: "s3://casts/ghi789.cast" });
append("intent-checker", { kind: "ArtifactValidated", dispatchId, artifactHash: "ghi789", artifactType: "cast", verdict: "accepted" });

append("claude-builder", { kind: "ArtifactUploaded", dispatchId, artifactType: "manual-review", hash: "jkl111", uri: "internal://review/1" });
append("alice-reviewer", { kind: "ArtifactValidated", dispatchId, artifactHash: "jkl111", artifactType: "manual-review", verdict: "accepted" });
append("bob-reviewer",   { kind: "ArtifactValidated", dispatchId, artifactHash: "jkl111", artifactType: "manual-review", verdict: "accepted" });
...
append("rogue-agent", { kind: "ArtifactUploaded", dispatchId, artifactType: "code", hash: "evil111", uri: "s3://evil/malware.tar.gz" });

// Attack 2: rogue tries to validate an artifact it doesn't have authority over
append("rogue-agent", { kind: "ArtifactValidated", dispatchId, artifactHash: "abc123", artifactType: "code", verdict: "accepted" });

// Attack 3: rogue tries to accept a proof (only jacob-laptop holds 'owner')
const dispatchId2 = "d-ship-2";
append("jacob-laptop", { kind: "DispatchProposed", dispatchId: dispatchId2, skillId: "ship-feature", payload: { spec: "LINEAR-999" } });
...
console.log(`   Dispatch ${dispatchId}: ${log.some(f => f.payload.kind === "DispatchConfirmed" && f.payload.dispatchId === dispatchId && !f.rejectedReason) ? "✅ CONFIRMED (proof complete, signatures verified)" : "❌ not confirmed"}`);
console.log(`   Dispatch ${dispatchId2}: ${log.some(f => f.payload.kind === "DispatchConfirmed" && f.payload.dispatchId === dispatchId2 && !f.rejectedReason) ? "✅ confirmed" : "❌ NOT CONFIRMED (all forgery attempts rejected)"}`);
